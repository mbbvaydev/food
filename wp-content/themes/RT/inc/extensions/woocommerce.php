...
// }
 remove_action( 'tooltip_woocommerce_before_shop_loop_item_title', 'woocommerce_show_product_loop_sale_flash', 10 );
...
if (! function_exists('rt_price_in_single')) {
	function rt_price_in_single(){
		?>
...
add_action('woocommerce_single_product_summary','rt_price_in_single',6);
...
function rt_tooltip_woocommerce_template_loop_product_thumbnail() {
	...
	<a class='rt-tooltip' data-tooltip='{\"image\": \"" . esc_attr( get_the_post_thumbnail_url( get_the_ID(), 'full' ) ) . "\"}' href='" . get_the_permalink() . "'>" . rt_woocommerce_get_product_thumbnail() . "</a></div>";
}
...
// add_action('woocommerce_after_shop_loop_item_title','rt_qv_woocommerce_template_single_excerpt',6);
...
add_action( 'woocommerce_after_single_product_summary', 'rt_woocommerce_single_product_summary', 9 );
